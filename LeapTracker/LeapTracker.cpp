...
float LeapTracker::calculateMakeAFistMetric(const LEAP_HAND* hand) {
    ...
    // Expand the range by adjusting the normalization
    float normalizedFlexion = (totalFlexion - minFlexion) / (maxFlexion - minFlexion);
    
    // Optional: Apply a power function to emphasize lower values
    //float power = 1.5f; // Adjust this value to fine-tune the curve
    //normalizedFlexion = std::pow(normalizedFlexion, power);
    
    return std::min(1.0f, std::max(0.0f, normalizedFlexion));
}
...
float LeapTracker::calculatePronationSupinationMetric(const LEAP_HAND* hand) {
    ...
    // Normalize angle to 0-1 range, where 0 is full pronation and 1 is full supination
...
float LeapTracker::calculateWristAROMMetric(const LEAP_HAND* hand) {
    ...
    LEAP_VECTOR normalizedWristDirection = {
        wristDirection.x / wristMagnitude,
        wristDirection.y / wristMagnitude,
        wristDirection.z / wristMagnitude
    };
    
    LEAP_VECTOR normalizedPalmDirection = {
        palmDirection.x / palmMagnitude,
        palmDirection.y / palmMagnitude,
        palmDirection.z / palmMagnitude
    };
    
    float dotProduct = normalizedWristDirection.x * normalizedPalmDirection.x + 
                       normalizedWristDirection.y * normalizedPalmDirection.y + 
                       normalizedWristDirection.z * normalizedPalmDirection.z;
    ...
    float upDownFactor = normalizedPalmDirection.y > 0 ? 1.0f : -1.0f;
    
    // Map the range [-1, 1] to [0, 1]
    return std::max(0.0f, std::min(1.0f, (normalizedAngle * upDownFactor + 1.0f) / 2.0f));
}
